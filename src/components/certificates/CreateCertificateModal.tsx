...
const CreateCertificateModal = ({
  ...
  const handleProjectChange = (projectId: string) => {
    ...
    if (projectId && formContractor) {
      loadProjectItems(projectId, formContractor);
  ...
  const handleContractorChange = (name: string) => {
    ...
    if (formProjectId && name) {
      loadProjectItems(formProjectId, name);
  ...
  const handleCreateCertificate = async () => {
    if (!userId || !formProjectId || !formContractor) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999]" role="dialog" aria-modal="true">
      ...
      {/* Modal Content */}
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-background border rounded-lg shadow-2xl p-6 z-[10000]" dir={isArabic ? "rtl" : "ltr"}>
        {/* Header */}
