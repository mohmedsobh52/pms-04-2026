  ...
  const exportPDF=useCallback(()=>{
    try{
      ...
      if(baselinesCompare&&baselinesCompare.length){
        ...
        autoTable(doc,{
          startY:60,
          head:[["Snapshot","Date","BAC Δ","EAC Δ","AC Δ","CPI Δ","SPI Δ","Time Δ (mo)"]],
...
⏱ التوصية للإدارة: مراجعة فورية للأنشطة المتأثرة في الاجتماع الأسبوعي القادم، وتحديث خطة الاسترداد خلال 5 أيام عمل.`;
  ...
  const regenerateCashFlowFromDates=useCallback((opts={})=>{
    ...
    const blocking=errs.filter(m=>!m.includes("في التدفق النقدي خارج نطاق"));
    if(blocking.length){
      if(opts.silent) return; // never auto-regen on invalid input
  ...
  // ── Auto-regenerate CF rows when project dates/duration change (debounced, silent) ──
  useEffect(()=>{
    const s=parseIso(project.startDate),e=parseIso(project.endDate);
    const dur=Number(project.duration)||0;
    if(!s||!e||dur<=0||e<=s)return;
    const t=setTimeout(()=>regenerateCashFlowFromDates({distributePV:false,silent:true}),700);
  ...
  // Auto-generated recommendations for cashflow tab
  const cfRecommendations=useMemo(()=>{
    const recs=[];
    const {tPV,tAC,tEV,fAC}=cfStats;
    const cpi=tAC>0?tEV/tAC:0;
    const spi=tPV>0?tEV/tPV:0;
    const nfRec=new Intl.NumberFormat("en-US",{maximumFractionDigits:0});
    const fmtR=v=>nfRec.format(Math.round(v*1e6));
    // Performance
    if(cpi>0&&cpi<0.85)recs.push({sev:"crit",icon:"💸",text:`CPI = ${cpi.toFixed(2)} — تجاوز حاد في التكلفة. أوقف الالتزامات غير الحرجة وراجع EAC فوراً.`});
    else if(cpi>0&&cpi<0.95)recs.push({sev:"warn",icon:"💸",text:`CPI = ${cpi.toFixed(2)} — تجاوز في التكلفة. راجع بنود الصرف الكبرى وأعد تقدير EAC.`});
    if(spi>0&&spi<0.85)recs.push({sev:"crit",icon:"⏱",text:`SPI = ${spi.toFixed(2)} — تأخر حاد في الجدول. فعّل خطة التعافي وزد الموارد على المسار الحرج.`});
    else if(spi>0&&spi<0.95)recs.push({sev:"warn",icon:"⏱",text:`SPI = ${spi.toFixed(2)} — تأخر في الجدول. خصّص موارد إضافية للمسار الحرج.`});
    // Budget
    if(tAC>tPV*1.1)recs.push({sev:"crit",icon:"🚨",text:`الصرف الفعلي (${fmtR(tAC)}) يتجاوز المخطط (${fmtR(tPV)}) بنسبة ${(((tAC-tPV)/tPV)*100).toFixed(0)}%. اطلب موافقة على مراجعة الميزانية.`});
    if(tEV<tPV*0.9&&tPV>0)recs.push({sev:"warn",icon:"📉",text:`القيمة المكتسبة EV (${fmtR(tEV)}) أقل من المخطط بـ ${fmtR(tPV-tEV)}. ركّز على إنجاز الأنشطة الحرجة.`});
    // Forecast & liquidity
    if(forecastDeficit)recs.push({sev:"crit",icon:"⚠️",text:`عجز نقدي متوقع في ${forecastDeficit.label} بفجوة ${fmtR(forecastDeficit.gap)} — جهّز ترتيبات تمويل مسبقة.`});
    if(fAC>0&&fAC>(tPV-tAC)*1.2)recs.push({sev:"warn",icon:"🔮",text:`الصرف المتوقع للأشهر القادمة (${fmtR(fAC)}) أعلى من الرصيد المتبقي للمخطط. أعد توزيع PV أو خفّض النطاق.`});
    // OK state
    if(cpi>=1&&spi>=1&&tPV>0)recs.push({sev:"ok",icon:"✅",text:"الأداء النقدي ضمن الأهداف — حافظ على نمط الصرف الحالي وراقب المخاطر الناشئة."});
    // Data quality checks
    if(cf.some(c=>c.evM>c.acM*1.5&&c.acM>0))recs.push({sev:"info",icon:"🔍",text:"بعض الأشهر تظهر EV أعلى بكثير من AC — تحقق من دقة بيانات الصرف الفعلي."});
    if(cf.some(c=>!c.isForecast&&c.pvM>0&&c.evM===0&&c.acM===0))recs.push({sev:"info",icon:"📝",text:"توجد أشهر مخططة بدون أي صرف أو إنجاز — حدّث البيانات لتعكس الواقع."});
    if(cf.some(c=>c.acM>c.pvM*2&&c.pvM>0))recs.push({sev:"warn",icon:"📊",text:"شهر واحد على الأقل يتجاوز فيه AC ضعف PV — تحقق من الفواتير أو أعد توزيع الميزانية."});
    if(cf.some(c=>c.evM>c.pvM*1.2&&c.pvM>0))recs.push({sev:"info",icon:"⚡",text:"تقدّم أسرع من المخطط في بعض الأشهر — فرصة لتقصير الجدول الإجمالي."});
    // Cumulative spike detection
    const spikes=cf.filter((c,i)=>i>0&&cf[i-1].acM>0&&c.acM>cf[i-1].acM*2);
    if(spikes.length)recs.push({sev:"warn",icon:"📈",text:`قفزة مفاجئة في الصرف بشهر ${spikes[0].month} — راجع سبب الزيادة (مدفوعات موردين/مستخلصات).`});
    // Underutilization
    if(tEV>0&&tEV<tAC*0.7)recs.push({sev:"crit",icon:"💔",text:`القيمة المكتسبة أقل من 70% من الصرف الفعلي — هدر واضح في الموارد، راجع الإنتاجية.`});
    // Burn rate
    const monthsElapsed=cf.filter(c=>!c.isForecast).length;
    if(monthsElapsed>0&&tPV>0){
      const burnRate=tAC/monthsElapsed;
      const remaining=cf.filter(c=>c.isForecast).reduce((s,c)=>s+c.pvM,0);
      if(burnRate*cf.filter(c=>c.isForecast).length>remaining*1.15&&remaining>0)
        recs.push({sev:"warn",icon:"🔥",text:`معدل الحرق الحالي (${fmtR(burnRate)}/شهر) سيتجاوز المخطط المتبقي بنهاية المشروع — أعد التخطيط.`});
    }
    // Variance trend
    const last3=cf.filter(c=>!c.isForecast).slice(-3);
    if(last3.length===3&&last3.every(c=>c.acM>c.pvM))recs.push({sev:"warn",icon:"📉",text:"تجاوز متواصل في AC على آخر 3 أشهر — اتجاه سلبي يحتاج تدخل."});
    if(last3.length===3&&last3.every(c=>c.evM<c.pvM*0.9))recs.push({sev:"warn",icon:"🐢",text:"إنجاز ضعيف متواصل على آخر 3 أشهر — راجع أداء المقاولين."});
    return recs;
  },[cfStats,forecastDeficit,cf]);
  ...
  const saveCF=id=>{
    // cfBuf holds RAW currency values (not millions). Convert to millions for internal storage.
    const e=validateCF(cfBuf);
    if(Object.keys(e).length){setCfErrs(e);return;}
    const before=cf.find(c=>c.id===id);
    const nv={pvM:+cfBuf.pvM/1e6,acM:+cfBuf.acM/1e6,evM:+cfBuf.evM/1e6};
    setCf(p=>p.map(c=>c.id===id?{...c,...nv}:c));
    setCfEditId(null);setCfErrs({});
    try{
      const diff=nv.pvM-nv.acM;
      const label=before?.label||before?.month||id;
      const nf=new Intl.NumberFormat("en-US",{maximumFractionDigits:0});
      toast.success(`✅ تم حفظ تدفق ${label}`,{description:`PV ${nf.format(nv.pvM*1e6)} · AC ${nf.format(nv.acM*1e6)} · EV ${nf.format(nv.evM*1e6)}`});
      logChange&&logChange(`تعديل تدفق نقدي (${label}): PV=${nv.pvM*1e6} AC=${nv.acM*1e6} EV=${nv.evM*1e6}`,"edit");
      if(before){
        const changed=Math.abs((before.pvM-nv.pvM))+Math.abs((before.acM-nv.acM))+Math.abs((before.evM-nv.evM));
        if(changed>0) addNotif&&addNotif(`💾 تم تحديث تدفق ${label} (Δ ${nf.format(changed*1e6)})`,diff<0?"warn":"info");
      }
    }catch(_){/* noop */}
  };
  const addRisk=()=>{
    ...
    <div dir="rtl" className={`evm-density-${density}`} style={{display:"flex",height:"100vh",fontFamily:"'Manrope','Cairo','Segoe UI',sans-serif",background:darkMode?"#0f172a":"hsl(var(--background))",color:darkMode?"#f1f5f9":"hsl(var(--foreground))",fontSize:13,overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflowY:"auto",overflowX:"hidden",position:"relative"}}>
        ...
        <header role="banner" aria-label="رأس صفحة التحكم في التكلفة" className="cc-header-pad" style={{background:"var(--gradient-hero)",padding:"12px 20px 0",color:"hsl(var(--primary-foreground))",flexShrink:0,boxShadow:"var(--shadow-md)",borderBottom:"1px solid hsla(0,0%,100%,.08)",position:"sticky",top:0,zIndex:50,overflow:"hidden"}}>
                    ...
                    {globalResults.length>0?(
                      <div>{globalResults.map((r,i)=>(
                        <div key={i} onClick={()=>{r.action();setShowGlobalSearch(false);setGlobalSearch("");}}
                          ...
                          <div><div style={{fontSize:12,fontWeight:600,color:darkMode?"#f1f5f9":"#1a1a2e"}}>{r.label}</div>
                          <div style={{fontSize:10,color:darkMode?"#94a3b8":"#888"}}>{r.sub}</div></div>
                        </div>))}
                      </div>
                    ):globalSearch.length>=2?(
                      <div style={{padding:"16px",textAlign:"center",color:"#888",fontSize:12}}>لا توجد نتائج</div>
                    ):(<div style={{padding:"12px 16px",fontSize:10,color:"#aaa"}}>اكتب للبحث · Ctrl+K للفتح/الإغلاق</div>)}
                  </div>
                )}
              </div>
                  ...
                  <div style={{position:"absolute",top:36,insetInlineEnd:0,width:300,background:darkMode?"#1e293b":"#fff",borderRadius:10,boxShadow:"0 20px 40px rgba(0,0,0,.3)",zIndex:200,border:`1px solid ${darkMode?"#334155":"#e5e7eb"}`,overflow:"hidden"}}>
                    ...
                    {notifications.slice(0,6).map(n=>(
                      <div key={n.id} onClick={()=>setNotifications(p=>p.map(x=>x.id===n.id?{...x,read:true}:x))}
                        ...
                        <div style={{flex:1}}><div style={{fontSize:11,fontWeight:n.read?400:700,color:darkMode?"#f1f5f9":"#333"}}>{n.msg}</div>
                        <div style={{fontSize:9,color:"#aaa",marginTop:2}}>{n.ts}</div></div>
          ...
          {/* Tabs */}
          <div style={{display:"flex",gap:2,flexWrap:"nowrap",overflowX:"auto",borderBottom:"1px solid hsla(0,0%,100%,.1)"}}>
            {TABS.map(({k,lbl})=>(
              <button key={k} onClick={()=>setTab(k)} style={{background:tab===k?"hsla(0,0%,100%,.16)":"transparent",color:"#fff",border:"1px solid transparent",borderBottom:tab===k?"2px solid hsl(var(--accent))":"2px solid transparent",borderRadius:"7px 7px 0 0",padding:"7px 14px",fontWeight:tab===k?700:500,cursor:"pointer",fontSize:11,whiteSpace:"nowrap",opacity:tab===k?1:.78,transition:"opacity .15s,background .15s"}}>
                {lbl}
              </button>
            ))}
          </div>
          ...
          <div style={{background:timeMetrics.overdue?"hsl(var(--destructive)/.08)":(darkMode?"#0f172a":"hsl(var(--card))"),borderBottom:`1px solid ${timeMetrics.overdue?"hsl(var(--destructive)/.3)":"hsl(var(--border))"}`,padding:"8px 20px",flexShrink:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,fontSize:11,fontWeight:700,color:darkMode?"#cbd5e1":"hsl(var(--foreground))",gap:12,flexWrap:"wrap"}}>
              <span style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                ...
                <b style={{fontFamily:"monospace",color:"hsl(var(--primary))"}}>{project.startDate||"—"}</b>
                <span>→</span>
                <b style={{fontFamily:"monospace",color:"hsl(var(--primary))"}}>{project.endDate||"—"}</b>
                <span style={{color:"hsl(var(--muted-foreground))"}}>·</span>
                <span>المدة: <b>{project.duration||"—"} شهر</b> {(()=>{const s=parseIso(project.startDate),e=parseIso(project.endDate);return s&&e?<span style={{color:"hsl(var(--muted-foreground))"}}>(<b style={{color:"hsl(var(--accent))"}}>{Math.max(1,Math.round((e-s)/86400000))}</b> يوم)</span>:null;})()}</span>
              </span>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 20px",background:darkMode?"#0f172a":"#f4f5fb"}}>
          ...
          {tab==="cashflow"&&(<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
              <Card>
                <H3>📊 التدفق النقدي الشهري (ريال)</H3>
                ...
                <div style={{fontSize:11,color:darkMode?"#cbd5e1":"#475569",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  ...
                  <label style={{display:"flex",alignItems:"center",gap:4}}>
                    ...
                    <input
                      ...
                      onChange={e=>{
                        ...
                        setProject(p=>recomputeProjectDates({...p,startDate:v,lockedField:"endDate"}));
                        const s=parseIso(v),en=parseIso(project.endDate);
                        if(s&&en&&en<=s) toast.error("⚠️ تاريخ النهاية يجب أن يكون بعد البداية");
                      }}
                  ...
                  <label style={{display:"flex",alignItems:"center",gap:4}}>
                    ...
                    <input
                      ...
                      onChange={e=>{
                        const v=e.target.value;
                        const s=parseIso(project.startDate),en=parseIso(v);
                        if(s&&en&&en<=s){ toast.error("⚠️ تاريخ النهاية يجب أن يكون بعد البداية"); return; }
                        setProject(p=>recomputeProjectDates({...p,endDate:v,lockedField:"startDate"}));
                      }}
                      style={{width:118,padding:"4px 6px",fontSize:11,fontFamily:"monospace",borderRadius:5,border:`1px solid ${darkMode?"#475569":"#cbd5e1"}`,background:darkMode?"#0f172a":"#fff",color:darkMode?"#f1f5f9":"#0f172a"}}
                    />
                  </label>
                  ...
                  <label style={{display:"flex",alignItems:"center",gap:4}}>
                    ...
                    <input
                      ...
                      min={1}
                      max={3650}
                      value={(()=>{const s=parseIso(project.startDate),e=parseIso(project.endDate);return s&&e?Math.max(1,Math.round((e-s)/86400000)):"";})()}
                      onChange={e=>{
                        const days=Math.max(1,Number(e.target.value)||0);
                        const s=parseIso(project.startDate);
                        if(!s||!days)return;
                        const newEnd=new Date(s);newEnd.setDate(newEnd.getDate()+days);
                        setProject(p=>recomputeProjectDates({...p,endDate:toIso(newEnd),lockedField:"duration"}));
                      }}
                      style={{width:64,padding:"4px 6px",fontSize:11,fontFamily:"monospace",borderRadius:5,border:`1px solid ${darkMode?"#475569":"#cbd5e1"}`,background:darkMode?"#0f172a":"#fff",color:darkMode?"#f1f5f9":"#0f172a",textAlign:"center"}}
                    />
                    <span>يوم</span>
                  </label>
                  <span style={{color:"hsl(var(--muted-foreground))"}}>·</span>
            ...
            <Card style={{padding:0,overflow:"hidden"}}>
              ...
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                  ...
                  <tbody>
                    {(()=>{const nfRaw=new Intl.NumberFormat("en-US",{maximumFractionDigits:0});return cfCum.map((c,idx)=>{
                    ...
                    {(()=>{const nfT=new Intl.NumberFormat("en-US",{maximumFractionDigits:0});return(
                    <tr style={{background:"#0f172a",color:"#fff",fontWeight:700}}>
                      ...
                      <td style={{padding:"9px 12px",textAlign:"right",fontFamily:"monospace",color:"#a5b4fc"}}>{nfT.format(Math.round(cfStats.tPV*1e6))}</td>
                      <td style={{padding:"9px 12px",textAlign:"right",fontFamily:"monospace",color:"#fde68a"}}>{nfT.format(Math.round(cfStats.tAC*1e6))}</td>
                      <td style={{padding:"9px 12px",textAlign:"right",fontFamily:"monospace",color:"#86efac"}}>{nfT.format(Math.round(cfStats.tEV*1e6))}</td>
                      <td colSpan={5}/>
                    </tr>);})()}
                  </tbody>
                </table>
            ...
            <Card style={{marginTop:12,background:"linear-gradient(135deg,hsl(var(--accent)/.06),hsl(var(--primary)/.04))"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8,flexWrap:"wrap",gap:8}}>
                <H3 style={{margin:0,display:"flex",alignItems:"center",gap:6}}>💡 اقتراحات وتوصيات تلقائية للتدفق النقدي
                  <span style={{background:"hsl(var(--primary)/.15)",color:"hsl(var(--primary))",borderRadius:999,padding:"1px 8px",fontSize:10,fontWeight:700}}>{cfRecommendations.length}</span>
                </H3>
                <span style={{fontSize:10,color:"hsl(var(--muted-foreground))"}}>تُحدَّث تلقائياً بعد أي تعديل في الجدول</span>
              </div>
              {cfRecommendations.length===0?(
                <div style={{fontSize:12,color:"hsl(var(--muted-foreground))",padding:"8px 4px"}}>لا توجد توصيات حالياً — الأداء النقدي ضمن الحدود الطبيعية.</div>
              ):(
                <div style={{display:"grid",gap:6}}>
                  {cfRecommendations.map((r,i)=>{
                    const bg=r.sev==="crit"?"hsl(var(--destructive)/.08)":r.sev==="warn"?"hsl(38 92% 50% / .08)":r.sev==="ok"?"hsl(160 84% 39% / .08)":"hsl(217 91% 60% / .08)";
                    const bd=r.sev==="crit"?"hsl(var(--destructive)/.3)":r.sev==="warn"?"hsl(38 92% 50% / .3)":r.sev==="ok"?"hsl(160 84% 39% / .3)":"hsl(217 91% 60% / .3)";
                    const fg=r.sev==="crit"?"hsl(var(--destructive))":r.sev==="warn"?"hsl(38 92% 35%)":r.sev==="ok"?"hsl(160 84% 30%)":"hsl(217 91% 45%)";
                    return(
                      <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",background:bg,border:`1px solid ${bd}`,borderRadius:8,padding:"8px 12px",fontSize:12,color:fg,fontWeight:600}}>
                        <span style={{fontSize:16,lineHeight:1}}>{r.icon}</span>
                        <span style={{flex:1,lineHeight:1.55}}>{r.text}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <Card style={{marginTop:14}}>
              ...
              {!mcResult?(
                ...
                <>
                  ...
                  {histView && (() => {
                    const {viewHist,total,overBudgetPct,iqrLow,iqrHigh,vaR} = histView;

                    // Compare-mode: project snapshot B onto same X bins as current viewHist
                    const cmpMc = compareMode ? mcSnapshots.B?.mc : null;
                    const cmpAMc = compareMode ? (mcSnapshots.A?.mc || mcResult) : null;
                    const overlay = cmpMc && viewHist.length ? viewHist.map((b,i,arr)=>{
                      const next = arr[i+1]?.mid ?? (b.mid + (b.mid - (arr[i-1]?.mid ?? b.mid)));
                      const lo = i===0 ? -Infinity : (arr[i-1].mid + b.mid)/2;
                      const hi = i===arr.length-1 ? Infinity : (b.mid + next)/2;
                      const cnt = (cmpMc.eacs||[]).filter(x=>x>=lo && x<hi).length;
                      return {...b, cmpCount:cnt};
                    }) : viewHist;
                    ...
                    const RichTip = ({active,payload})=>{
                      ...
                      return (
                        <div style={{background:darkMode?"#0f172a":"#fff",border:`2px solid ${overBac?"#ef4444":"#10b981"}`,borderRadius:10,padding:"10px 12px",fontSize:11,minWidth:220,boxShadow:"0 8px 24px rgba(0,0,0,0.18)"}}>
                          ...
                          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:4,marginBottom:4}}>
                            <span style={{color:"#888"}}>{compareMode?`${mcSnapshots.A?.label||"A"} — عدد:`:"عدد المحاكاة:"}</span><b style={{fontFamily:"monospace"}}>{d.count}</b>
                            {compareMode && cmpMc && (<><span style={{color:"#888"}}>{mcSnapshots.B?.label||"B"} — عدد:</span><b style={{fontFamily:"monospace",color:"#f59e0b"}}>{d.cmpCount}</b></>)}
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12,marginBottom:12}}>
                    <Card>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:8}}>
                        <H3 style={{margin:0}}>📊 توزيع EAC (Histogram + CDF) {compareMode && <span style={{fontSize:11,color:"#f59e0b",fontWeight:700}}>· وضع المقارنة</span>}</H3>
                        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                          <button onClick={()=>exportChart("png")} title="تصدير الرسم كصورة PNG" style={{padding:"5px 10px",borderRadius:6,border:"1px solid #6366f1",background:"#6366f110",color:"#6366f1",fontSize:10,fontWeight:700,cursor:"pointer"}}>📥 PNG</button>
                          <button onClick={()=>exportChart("svg")} title="تصدير الرسم كملف SVG" style={{padding:"5px 10px",borderRadius:6,border:"1px solid #8b5cf6",background:"#8b5cf610",color:"#8b5cf6",fontSize:10,fontWeight:700,cursor:"pointer"}}>📥 SVG</button>
                          <button onClick={exportMcCSV} title="تصدير العينات + الإحصائيات CSV" style={{padding:"5px 10px",borderRadius:6,border:"1px solid #10b981",background:"#10b98110",color:"#10b981",fontSize:10,fontWeight:700,cursor:"pointer"}}>📊 CSV</button>
                          <button onClick={exportExecutivePDF} title="ملخص تنفيذي للمخاطر PDF" style={{padding:"5px 10px",borderRadius:6,border:"1px solid #ef4444",background:"#ef444410",color:"#ef4444",fontSize:10,fontWeight:700,cursor:"pointer"}}>📄 Exec PDF</button>
                        </div>
                      </div>

                      {/* Compare-mode toolbar */}
                      <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",padding:"6px 10px",background:darkMode?"#1e1b4b":"#faf5ff",borderRadius:7,marginBottom:8,fontSize:10,border:"1px dashed #c4b5fd"}}>
                        <span style={{fontWeight:700,color:"#7c3aed"}}>🔀 المقارنة:</span>
                        <button onClick={()=>saveMcSnapshot("A")} style={{padding:"3px 8px",borderRadius:5,border:"1px solid #6366f1",background:mcSnapshots.A?"#6366f120":"transparent",color:"#6366f1",fontWeight:700,cursor:"pointer"}}>💾 حفظ كـ A</button>
                        <button onClick={()=>saveMcSnapshot("B")} style={{padding:"3px 8px",borderRadius:5,border:"1px solid #f59e0b",background:mcSnapshots.B?"#f59e0b20":"transparent",color:"#f59e0b",fontWeight:700,cursor:"pointer"}}>💾 حفظ كـ B</button>
                        <button disabled={!mcSnapshots.A||!mcSnapshots.B} onClick={()=>setCompareMode(m=>!m)} style={{padding:"3px 8px",borderRadius:5,border:`1px solid ${compareMode?"#ef4444":"#10b981"}`,background:compareMode?"#ef444420":"#10b98110",color:compareMode?"#ef4444":"#10b981",fontWeight:700,cursor:(!mcSnapshots.A||!mcSnapshots.B)?"not-allowed":"pointer",opacity:(!mcSnapshots.A||!mcSnapshots.B)?0.4:1}}>{compareMode?"⏹ إيقاف المقارنة":"▶ تشغيل المقارنة"}</button>
                        {mcSnapshots.A && <span style={{color:"#6366f1"}}>A: <b>{mcSnapshots.A.label}</b></span>}
                        {mcSnapshots.B && <span style={{color:"#f59e0b"}}>B: <b>{mcSnapshots.B.label}</b></span>}
                        {(mcSnapshots.A||mcSnapshots.B) && <button onClick={()=>{setMcSnapshots({A:null,B:null});setCompareMode(false);}} style={{padding:"3px 6px",borderRadius:5,border:"1px solid #94a3b8",background:"transparent",color:"#64748b",fontSize:9,cursor:"pointer",marginInlineStart:"auto"}}>🗑 مسح</button>}
                      </div>
                      ...
                      <ResponsiveContainer width="100%" height={260}>
                        <ComposedChart data={overlay} margin={{top:10,right:30,left:0,bottom:30}}>
                          <defs>
                            ...
                            <linearGradient id="histCmp" x1="0" y1="0" x2="0" y2="1">
                          ...
                          <ReferenceLine yAxisId="left" x={`${(Math.floor(mcResult.bac/1e6*100)/100).toFixed(2)}M`} stroke="#ef4444" strokeWidth={2} strokeDasharray="5 3" label={{value:compareMode?`BAC-A (${mcResult.probOverBudget}%)`:`BAC (${mcResult.probOverBudget}% تجاوز)`,position:"top",fontSize:10,fill:"#ef4444",fontWeight:700}}/>
                          {compareMode && cmpMc && <ReferenceLine yAxisId="left" x={`${(Math.floor(cmpMc.bac/1e6*100)/100).toFixed(2)}M`} stroke="#f59e0b" strokeWidth={2} strokeDasharray="2 2" label={{value:`BAC-B (${cmpMc.probOverBudget}%)`,position:"top",fontSize:10,fill:"#f59e0b",fontWeight:700}}/>}
                          ...
                          <Bar yAxisId="left" dataKey="count" name={compareMode?(mcSnapshots.A?.label||"A"):"عدد المحاكاة"} radius={[4,4,0,0]}>
                            {overlay.map((d,i)=>(
                              <Cell key={i} fill={d.mid>mcResult.bac?"url(#histRisk)":"url(#histSafe)"}/>
                            ))}
                          </Bar>
                          {compareMode && cmpMc && <Bar yAxisId="left" dataKey="cmpCount" name={mcSnapshots.B?.label||"B"} fill="url(#histCmp)" radius={[4,4,0,0]} fillOpacity={0.75}/>}
                          <Line yAxisId="right" type="monotone" dataKey="cumPct" name="تراكمي A %" stroke="#8b5cf6" strokeWidth={2.5} dot={false} activeDot={{r:4}}/>
                        </ComposedChart>
                      </ResponsiveContainer>
                      </div>
                      ...
                      {/* Statistical glossary + quick interpretation */}
                      <details style={{marginTop:10,padding:"8px 10px",background:darkMode?"#0b1224":"#fefce8",border:"1px solid #fde68a",borderRadius:7,fontSize:11}}>
                        <summary style={{cursor:"pointer",fontWeight:800,color:"#a16207"}}>📖 شرح المؤشرات الإحصائية وتفسير القراءة الحالية</summary>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:10,lineHeight:1.7}}>
                          <div title="Inter-Quartile Range — عرض النطاق الذي يغطي 80% من السيناريوهات بين P10 و P90">
                            <b style={{color:"#8b5cf6"}}>IQR (P10↔P90):</b><br/>
                            <span style={{color:"#6b7280"}}>عرض نطاق الثقة 80% — كلما اتسع كلما زاد عدم اليقين.</span><br/>
                            <span style={{fontFamily:"monospace",color:"#8b5cf6"}}>= {fmt(iqrHigh-iqrLow)} ({((iqrHigh-iqrLow)/mcResult.bac*100).toFixed(1)}% من BAC)</span>
                          </div>
                          <div title="Value at Risk — حجم التعرض الإضافي في الذيل العلوي للتوزيع">
                            <b style={{color:"#6366f1"}}>VaR (P90 − Mean):</b><br/>
                            <span style={{color:"#6b7280"}}>التعرض الإضافي للمخاطر فوق المتوسط في أسوأ 10%.</span><br/>
                            <span style={{fontFamily:"monospace",color:"#6366f1"}}>= {fmt(vaR)} ({(vaR/mcResult.bac*100).toFixed(1)}% من BAC)</span>
                          </div>
                          <div title="Cumulative Distribution Function — احتمال أن لا يتجاوز EAC قيمة معينة">
                            <b style={{color:"#a855f7"}}>CDF (المنحنى البنفسجي):</b><br/>
                            <span style={{color:"#6b7280"}}>نسبة المحاكاة التي EAC فيها ≤ القيمة المقابلة على المحور.</span><br/>
                            <span style={{fontFamily:"monospace",color:"#a855f7"}}>عند BAC: ~{(100-mcResult.probOverBudget).toFixed(1)}%</span>
                          </div>
                          <div title="Box-Muller — مولّد عشوائي يستنسخ توزيعاً طبيعياً حول CPI/SPI الحاليتين">
                            <b style={{color:"#10b981"}}>📌 التفسير السريع:</b><br/>
                            <span style={{color:"#6b7280"}}>
                              {overBudgetPct>50?"الغالبية تتجاوز BAC — تدخل عاجل مطلوب.":
                               overBudgetPct>20?"احتمال محسوس للتجاوز — راقب وحضّر خطة تخفيف.":
                               "الوضع آمن نسبياً — استمر بالضوابط الحالية."}
                            </span>
                          </div>
                        </div>
                      </details>
                    ...
                    <Card>
                      ...
                      <table style={{width:"100%",fontSize:11,borderCollapse:"collapse"}}>
                        ...
                        <tfoot>
                          <tr style={{background:"#8b5cf615"}}>
                            <td title="Inter-Quartile Range = P90 - P10 — عرض النطاق الذي يحتوي 80% من السيناريوهات" style={{padding:"7px 8px",fontWeight:800,color:"#8b5cf6",cursor:"help"}}>IQR (P10↔P90) ⓘ</td>
                          ...
                          <tr style={{background:"#6366f115"}}>
                            <td title="Value at Risk = P90 - Mean — حجم التعرض في ذيل التوزيع فوق المتوسط" style={{padding:"7px 8px",fontWeight:800,color:"#6366f1",cursor:"help"}}>VaR (P90 − Mean) ⓘ</td>
                      ...
                      {/* Compare summary when active */}
                      {compareMode && cmpMc && (
                        <div style={{marginTop:10,padding:"8px 10px",background:"#fef3c7",border:"1px solid #fcd34d",borderRadius:7,fontSize:10,lineHeight:1.7}}>
                          <div style={{fontWeight:800,color:"#a16207",marginBottom:4}}>🔀 مقارنة A vs B</div>
                          <div>P50: <b style={{color:"#6366f1"}}>{fmt(mcResult.eac.p50)}</b> → <b style={{color:"#f59e0b"}}>{fmt(cmpMc.eac.p50)}</b> ({((cmpMc.eac.p50-mcResult.eac.p50)/mcResult.eac.p50*100).toFixed(1)}%)</div>
                          <div>P90: <b style={{color:"#6366f1"}}>{fmt(mcResult.eac.p90)}</b> → <b style={{color:"#f59e0b"}}>{fmt(cmpMc.eac.p90)}</b></div>
                          <div>احتمال التجاوز: <b style={{color:"#6366f1"}}>{mcResult.probOverBudget}%</b> → <b style={{color:"#f59e0b"}}>{cmpMc.probOverBudget}%</b></div>
                        </div>
                      )}
                    </Card>
                  </div>
