...
⏱ التوصية للإدارة: مراجعة فورية للأنشطة المتأثرة في الاجتماع الأسبوعي القادم، وتحديث خطة الاسترداد خلال 5 أيام عمل.`;
  ...
  const cfRecommendations=useMemo(()=>{
    ...
    if(cpi>0&&cpi<0.95)recs.push({sev:"warn",icon:"💸",text:`CPI = ${cpi.toFixed(2)} — تجاوز في التكلفة. راجع بنود الصرف الكبرى وأعد تقدير EAC.`});
    if(spi>0&&spi<0.95)recs.push({sev:"warn",icon:"⏱",text:`SPI = ${spi.toFixed(2)} — تأخر في الجدول. خصّص موارد إضافية للمسار الحرج.`});
    if(tAC>tPV*1.1)recs.push({sev:"crit",icon:"🚨",text:`الصرف الفعلي (${tAC.toFixed(1)}M) يتجاوز المخطط (${tPV.toFixed(1)}M) بنسبة ${(((tAC-tPV)/tPV)*100).toFixed(0)}%. اطلب موافقة على مراجعة الميزانية.`});
    if(tEV<tPV*0.9&&tPV>0)recs.push({sev:"warn",icon:"📉",text:`القيمة المكتسبة EV (${tEV.toFixed(1)}M) أقل من المخطط بـ ${(tPV-tEV).toFixed(1)}M. ركّز على إنجاز الأنشطة الحرجة.`});
    if(forecastDeficit)recs.push({sev:"crit",icon:"⚠️",text:`عجز نقدي متوقع في ${forecastDeficit.label} بفجوة ${forecastDeficit.gap}M — جهّز ترتيبات تمويل مسبقة.`});
    if(fAC>0&&fAC>(tPV-tAC)*1.2)recs.push({sev:"warn",icon:"🔮",text:`الصرف المتوقع للأشهر القادمة (${fAC.toFixed(1)}M) أعلى من الرصيد المتبقي للمخطط. أعد توزيع PV أو خفّض النطاق.`});
