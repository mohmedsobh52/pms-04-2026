...
⏱ التوصية للإدارة: مراجعة فورية للأنشطة المتأثرة في الاجتماع الأسبوعي القادم، وتحديث خطة الاسترداد خلال 5 أيام عمل.`;
  ...
  const saveCF=id=>{
    ...
    const nv={pvM:+cfBuf.pvM,acM:+cfBuf.acM,evM:+cfBuf.evM};
    setCf(p=>p.map(c=>c.id===id?{...c,...cfBuf,...nv}:c));
    setCfEditId(null);setCfErrs({});
    try{
      const diff=nv.pvM-nv.acM;
      const label=before?.label||before?.month||id;
      toast.success(`✅ تم حفظ تدفق ${label}`,{description:`PV ${nv.pvM.toFixed(1)}M · AC ${nv.acM.toFixed(1)}M · EV ${nv.evM.toFixed(1)}M`});
      logChange&&logChange(`تعديل تدفق نقدي (${label}): PV=${nv.pvM} AC=${nv.acM} EV=${nv.evM}`,"edit");
      if(before){
        const changed=Math.abs((before.pvM-nv.pvM))+Math.abs((before.acM-nv.acM))+Math.abs((before.evM-nv.evM));
        if(changed>0) addNotif&&addNotif(`💾 تم تحديث تدفق ${label} (Δ ${changed.toFixed(1)}M)`,diff<0?"warn":"info");
      }
    }catch(_){/* noop */}
  };
  const addRisk=()=>{
